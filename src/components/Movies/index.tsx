...
const Allmovie = () => {
  ...
  <Topnav />
        <div className="mt-32">  <Navbar /></div>
              ...
              {movies.map((movie) => (
                ...
                title={movie.title}
                >
                <Link href={`/movie/${movie.id}`} className="w-full flex justify-center group relative" title={`View details for ${movie.title}`}>
                <Image
                  src={
                  movie.poster_path
                ...
                <span className="mt-4 text-center font-semibold text-gray-200 text-base line-clamp-2 group-hover:text-yellow-200 transition" title={movie.title}>
                {movie.title}
                </span>
                <span className="text-sm text-gray-300 mb-2 group-hover:text-yellow-100 transition" title={`Release date: ${movie.release_date}`}>
                {movie.release_date}
                </span>
                <button
                ...
                title={user?.watchlist?.includes(movie.id) ? "Already in Watchlater" : "Mark as seen"}
                >
                ...
                title={user?.watchlist?.includes(movie.id) ? "Already added to Watch Later" : "Add to Watch Later"}
                >
                {user?.watchlist?.includes(movie.id) ? (
                <>
                <svg
